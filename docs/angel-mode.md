...
Flight, landing, walking/running, the joystick and directional buttons are grouped under **Character**. Camera presets and fullscreen are under **View**; pause, audio and settings are under **System**. Close the dropdown to clear the scene or resume keyboard movement. Opening another category releases held touch controls, and the selected planet is retained when the dropdown closes.
