...
export function TouchControls({onMove,disabled=false}:{onMove:(x:number,z:number)=>void;disabled?:boolean}){
  ...
  useEffect(()=>{const hidden=()=>{if(document.hidden)clear()};window.addEventListener('blur',clear);window.addEventListener('resize',clear);document.addEventListener('visibilitychange',hidden);return()=>{window.removeEventListener('blur',clear);window.removeEventListener('resize',clear);document.removeEventListener('visibilitychange',hidden);clear()}},[]);
