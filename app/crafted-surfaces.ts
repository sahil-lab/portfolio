...
export function craftedBox(w:number,h:number,d:number){
  ...
  return shortest<.22||Math.max(w,h,d)>100
    ...
    :new RoundedBoxGeometry(w,h,d,1,Math.min(shortest*.18,.16));
}
...
export function createCraftMaterials(){
  ...
  for(let y=0;y<size;y++)for(let x=0;x<size;x++){
    ...
    const value=Math.round(210+grain*19+Math.sin(y*.54+x*.07)*5);
  ...
  return (color:string,glow=0,metalness=.12)=>new T.MeshStandardMaterial({
    color,emissive:color,emissiveIntensity:glow,metalness,
    roughness:metalness>.3?.42:.72,roughnessMap:grain,bumpMap:grain,bumpScale:.018,
    envMapIntensity:metalness>.3?.65:.3,
  });
}
