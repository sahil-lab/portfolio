...
export function buildWorldScenery(scene:T.Scene){
 ...
 const cyl=(x:number,y:number,z:number,r:number,h:number,c:string,g:T.Object3D=scene)=>{if(g===scene&&h>1.1&&y>0)physicalBoxes.push({x,y,z,w:r*2,h,d:r*2});return mesh(new T.CylinderGeometry(r,r,h,32),c,x,y,z,g)};
 ...
 box(0,-1.2,44,110,2,194,'#204e48');box(0,-2.7,44,114,1,198,'#142e30');
 for(let i=0;i<18;i++){const x=-51+i*6;line([new T.Vector3(x,-.12,50),new T.Vector3(x,-.12,10+i%5),new T.Vector3(x+5,-.12,5+i%5),new T.Vector3(x+5,-.12,-49)],i%3?'#467061':'#ae8b50',.035);cyl(x,-.08,43-i%4,.18,.08,'#b39c62')}
 ...
 for(let i=0;i<3;i++)line([new T.Vector3(-57,19+i,-35+i*12),new T.Vector3(0,15+i,-25+i*12),new T.Vector3(56,24+i,-20+i*12)],i%2?'#475e51':'#9b7e50',.25);
 for(let i=0;i<12;i++){const x=Math.sin(i*72)*49,z=Math.cos(i*33)*46;if(districts.some(d=>Math.hypot(d.x-x,d.z-z)<10))continue;cyl(x,1.2,z,.65,2.4,'#294641');cyl(x,2.45,z,.65,.1,'#b8b79a');for(let j=0;j<3;j++)ball(x+j*.6,.5,z+1,.23,'#69947c')}
