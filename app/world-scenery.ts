...
export function buildWorldScenery(scene:T.Scene){
 const mat=(c:string,em=0)=>new T.MeshStandardMaterial({color:c,roughness:.86,metalness:.06,emissive:c,emissiveIntensity:em});
 ...
 const box=(x:number,y:number,z:number,w:number,h:number,d:number,c:string,g:T.Object3D=scene,em=0)=>{const o=mesh(new T.BoxGeometry(w,h,d),c,x,y,z,g,em);o.userData.cameraSolid=h>.2;if((h>1.1||y-h/2>2.5)&&w>.1&&d>.1&&y>0&&g===scene)physicalBoxes.push({x,y,z,w,h,d});return o};
