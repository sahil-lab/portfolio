...
export function bindGameInput(canvas:HTMLCanvasElement,events:{interact:()=>void;pause:()=>void;look:(x:number,y:number)=>void;zoom:(n:number)=>void;select:(x:number,y:number)=>void;touchSelect?:(x:number,y:number)=>void;flight?:()=>boolean;gesture:()=>void}){
  ...
  const keydown=(e:KeyboardEvent)=>{if(editable(e.target))return;if(e.code==='KeyP'||e.code==='Escape'){e.preventDefault();if(!e.repeat)events.pause();return}if(!enabled)return;if(['w','a','s','d','arrowup','arrowdown','arrowleft','arrowright','e','shift'].includes(e.key.toLowerCase())||events.flight?.()&&[' ','q','control'].includes(e.key.toLowerCase()))e.preventDefault();state.keys.add(e.key.toLowerCase());events.gesture();if(e.key.toLowerCase()==='e'&&!e.repeat)events.interact()};
  ...
  const up=(e:PointerEvent)=>{if(state.pointers.has(e.pointerId)&&!moved&&enabled){if(e.pointerType==='touch')events.touchSelect?.(e.clientX,e.clientY);else events.select(e.clientX,e.clientY)}state.pointers.delete(e.pointerId);if(canvas.hasPointerCapture(e.pointerId))canvas.releasePointerCapture(e.pointerId);moved=true};
