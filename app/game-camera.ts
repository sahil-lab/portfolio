...
export function createGameCamera(camera:T.PerspectiveCamera,scene:T.Scene,player:T.Group){
  ...
  return {
    ...
    setMode:(value:CameraMode)=>{mode=value;pitch=mode==='first-person'?0:.42;stablePitch=pitch;stableYaw=yaw;zoom=mode==='far'?46:12;eased=zoom;distance=zoom;focusHeight=1.5},
    rotate:(x:number,y:number,stable:boolean)=>{if(stable)return;yaw-=x*.005;pitch=T.MathUtils.clamp(pitch+y*.004,mode==='first-person'?-1.35:.16,mode==='first-person'?1.35:1.15)},
    zoom:(amount:number)=>{if(mode!=='first-person')zoom=T.MathUtils.clamp(zoom+amount,5,160)},
    reset:(view:{yaw?:number;pitch?:number;zoom?:number;focusHeight?:number}={})=>{yaw=view.yaw??.1;pitch=view.pitch??(mode==='first-person'?0:.42);zoom=view.zoom??(mode==='far'?46:12);eased=zoom;distance=zoom;focusHeight=view.focusHeight??1.5;stableYaw=yaw;stablePitch=view.pitch??(mode==='first-person'?0:.38)},
