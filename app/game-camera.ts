...
export function createGameCamera(camera:T.PerspectiveCamera,scene:T.Scene,player:T.Group){
  let yaw=.1,pitch=.45,zoom=34,distance=34;
  ...
  return {
    ...
    zoom:(amount:number)=>{zoom=T.MathUtils.clamp(zoom+amount,5,80)},
    reset:()=>{yaw=.1;pitch=.45;zoom=34;distance=34},
    update:(dt:number,inside:boolean,settings:Settings,vehicle=false)=>{
      ...
      player.getWorldPosition(target);target.y+=1.5;
      const angle=settings.stableCamera?.38:pitch,rotation=settings.stableCamera?.1:yaw;
