...
export function createWorld(host:HTMLElement,callbacks:WorldCallbacks,initialSettings:Settings=defaultSettings,initialDelivery:DeliverySnapshot|null=null,createCharacter:typeof createCourier=createCourier){
 ...
 const angel=createFlyingAngel(scene,camera,{dogHeight:dog.height,surfaces:transport.surfaces,change:status=>callbacks.onAngel?.(status),
  ...
  blocked:(point,radius,stop)=>{
   ...
   for(let index=0;index<9;index++){
    ...
    for(const level of [.8,4,8,12,16,20,23.7]){const y=point.y+level;if(goldMonument.blocked(x,z,y)||city.blocked(x,z,y)||cityGardens.blocked(x,z,y)||workshop.blocked(x,z,y)||collideScenery(x,z,y)||awe.blocked(x,z,y)||weather.blocked(x,z,y)||commons.blocked(x,z,y)||bulletins.blocked(x,z,y)||artMoments.blocked(x,z,y)||chronicle.blocked(x,z,y))return true}
   }
   return false;
  },
 ...
 const input=bindGameInput(renderer.domElement,{interact,pause:()=>callbacks.onPauseToggle?.(),look:(x,y)=>angel.controlled?angel.look(x,y,settings.stableCamera):rig.rotate(x,y,settings.stableCamera),zoom:amount=>angel.controlled?angel.zoom(amount):rig.zoom(amount),gesture:audioGesture,flight:()=>angel.controlled,touchSelect:(x,y)=>{if(angel.select(pointerRay(x,y)))setAngelMode(true)},select:(x,y)=>{const ray=pointerRay(x,y);if(angel.select(ray)){setAngelMode(true);return}if(angel.controlled)return;if(transport.civilizationLink.select(ray)){callbacks.onInfo?.();return}buildings.select(ray)}});
 ...
 return {
  ...
  city,goCity:(id:CityDistrictKind='lantern')=>{if(angel.controlled||paused||transport.journey.mode)return false;const district=cityDistricts.find(district=>district.id===id);if(!district)return false;commons.voice.cancel();transport.home();player.position.set(id==='lantern'?cityArrival.x:district.x,.8,id==='lantern'?cityArrival.z:district.z+30);resetCamera(id==='lantern'?cityCameraView(camera.aspect):{yaw:.22,pitch:.48,zoom:Math.max(62,Math.min(130,54/camera.aspect)),focusHeight:6.5});input.state.clear();callbacks.onNotice?.(district.name);return true},
  bulletins,goBulletins:(kind:keyof typeof bulletinSites='markets')=>{if(angel.controlled||paused||transport.journey.mode)return false;commons.voice.cancel();transport.home();player.position.set(bulletinSites[kind].x,bulletinView.y,bulletinView.z);resetCamera(bulletinCameraView(camera.aspect));input.state.clear();void bulletinFeed?.refresh();callbacks.onNotice?.(kind==='markets'?'Market Watch':'World Business');return true},
