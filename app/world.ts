...
export function createWorld(host:HTMLElement,callbacks:WorldCallbacks,initialSettings:Settings=defaultSettings,initialDelivery:DeliverySnapshot|null=null,createCharacter:typeof createCourier=createCourier){
 ...
 const living=createLivingWorld(scene,player,courier,{...callbacks,initialDelivery});
 const rig=createGameCamera(camera,scene,player);
 const audioGesture=()=>{living.volume(settings.volume);living.sound(!settings.muted&&!paused)};
 ...
 function tick(){
  ...
  if(!paused&&!document.hidden){
   ...
   buildings.update(dt);const lifePrompt=living.update(dt,buildings.getInside()!==null,settings.reducedMotion);
 ...
 return {
  ...
  settings:(value:Settings)=>{const qualityChanged=settings.quality!==value.quality;settings={...value};living.volume(value.volume);if(qualityChanged){configure();resize()}if(value.muted)living.sound(false)},
  setPaused:(v:boolean)=>{paused=v;input.setEnabled(!paused&&!menuOpen);if(v)living.sound(false)},
