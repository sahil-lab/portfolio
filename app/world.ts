...
export function createWorld(host:HTMLElement,callbacks:WorldCallbacks,initialSettings:Settings=defaultSettings,initialDelivery:DeliverySnapshot|null=null,createCharacter:typeof createCourier=createCourier){
 ...
 const camera=new T.PerspectiveCamera(50,1,.1,4000);scene.add(new T.HemisphereLight('#fff0d6','#425862',.85));const sun=new T.DirectionalLight('#ffe1ad',2.1);sun.position.set(-25,55,25);sun.castShadow=true;sun.shadow.normalBias=.035;sun.shadow.mapSize.set(2048,2048);Object.assign(sun.shadow.camera,{left:-130,right:130,top:130,bottom:-130,far:500});scene.add(sun);const rim=new T.DirectionalLight('#a3cee7',.4);rim.position.set(30,25,-35);scene.add(rim);
 ...
 function sceneryBlocked(x:number,z:number){return collideScenery(x,z,player.position.y)||machines.blocked(x,z,player.position.y)}
 ...
 return {
  ...
  stick:(x:number,z:number)=>{if(!paused&&!menuOpen){input.state.stick={x,y:z};audioGesture()}},
    settings:(value:Settings)=>{const qualityChanged=settings.quality!==value.quality;settings={...value};living.volume(value.volume);audio.setVolume(value.volume);commons.speaker.sound(audioUnlocked&&!value.muted&&!paused,value.volume);if(qualityChanged){configure();resize()}if(value.muted){commons.voice.cancel();living.sound(false);audio.enable(false)}},
