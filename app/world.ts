...
export function createWorld(host:HTMLElement,callbacks:WorldCallbacks,initialSettings:Settings=defaultSettings,initialDelivery:DeliverySnapshot|null=null,createCharacter:typeof createCourier=createCourier){
 const audio=new KingdomAudio();const scene=new T.Scene();scene.background=new T.Color('#0a1e29');scene.fog=new T.FogExp2('#173741',.00225);
 const renderer=new T.WebGLRenderer({antialias:true});renderer.setPixelRatio(Math.min(devicePixelRatio,1.5));renderer.shadowMap.enabled=true;renderer.shadowMap.autoUpdate=false;renderer.shadowMap.needsUpdate=true;renderer.shadowMap.type=T.PCFShadowMap;renderer.toneMapping=T.ACESFilmicToneMapping;renderer.toneMappingExposure=1.1;host.appendChild(renderer.domElement);
 const studio=new RoomEnvironment(),prefilter=new T.PMREMGenerator(renderer),reflections=prefilter.fromScene(studio,.03);scene.environment=reflections.texture;scene.environmentIntensity=.22;studio.dispose();prefilter.dispose();
 const camera=new T.PerspectiveCamera(50,1,.1,1000);scene.add(new T.HemisphereLight('#fff0d6','#425862',.85));const sun=new T.DirectionalLight('#ffe1ad',2.1);sun.position.set(-25,55,25);sun.castShadow=true;sun.shadow.normalBias=.035;sun.shadow.mapSize.set(2048,2048);Object.assign(sun.shadow.camera,{left:-130,right:130,top:130,bottom:-130,far:500});scene.add(sun);const rim=new T.DirectionalLight('#a3cee7',.4);rim.position.set(30,25,-35);scene.add(rim);
 ...
 const buildings=addProjectBuildings(scene,player,{externalBlocked:(x,z)=>living.blocked(x,z)||muralBlocked(x,z)||sceneryBlocked(x,z)||awe.blocked(x,z,player.position.y),...callbacks,onPrompt:(text:string)=>{buildingPrompt=text}});
 ...
 let last=performance.now(),place=0,time=0,shadowClock=0,footDistance=0,ambientClock=0,overlookSeen=false;const priorPosition=player.position.clone();const priorStages=exhibits.map(e=>e.snapshot.step);
 function tick(){
  ...
  if(!paused&&!document.hidden){
   ...
   const riding=transport.update(dt,x,z,settings.reducedMotion);
   ...
   const prompt=transport.prompt()??traversal.prompt()??(buildings.getInside()!==null?buildingPrompt:machines.prompt()||lifePrompt||buildingPrompt);
  ...
  rig.update(dt,buildings.getInside()!==null,settings,!!transport.journey.mode||transport.driving);avatar.position.y=transport.driving?2.7:0;if(transport.inRocket)player.visible=false;shadowClock+=dt;if(shadowClock>.125){renderer.shadowMap.needsUpdate=true;shadowClock=0}renderer.render(scene,camera);if(!document.hidden)meter(raw);
 }
 ...
 const home=()=>{transport.home();player.position.set(workshopSpawn.x,workshopSpawn.y,workshopSpawn.z);place=0;callbacks.onPlace(0);rig.reset();input.state.clear()};
 callbacks.onPlace(0);tick();callbacks.onReady();
 return {
  transport,goTransitHub:()=>{transport.hub();rig.reset();input.state.clear()},
  startTransit:(destination:number,mode:'metro'|'rocket')=>{if(paused)return false;audioGesture();input.state.clear();return transport.start(destination,mode)},
  ...
  home,step:(dx:number,dz:number)=>{if(!paused&&!menuOpen&&!traversal.moving&&!transport.journey.mode){audioGesture();if(transport.driving)transport.update(.08,dx,dz,settings.reducedMotion);else moveCharacter(player,dx,dz,.75,transport.blocked,transport.height,transport.bounds)}},
  route:buildings.route,travel:(i:number)=>{if(paused||menuOpen||!districts[i])return;transport.home();player.position.set(districtDestinations[i].x,districtDestinations[i].y,districtDestinations[i].z);input.state.clear()},
